run.R2jags.model <- function(d,
  ...
  cat('model {
    ...
    for(sp in 1:nsp) {
      ...
      psi.0.sp[sp] <- logit(psi.0.sp.prior[sp]) # Fixed effect of species on intercept 
      											# independent of trait
      psi.0[sp] <- psi.0.sp[sp] + psi.0.trait*trait[sp] #Total effect of species on intercept
    }
    ...
    psi.beta.sp.rand[1:nsp] ~ dmnorm(zero.sp[], tau.beta.sp.mat[,])
    
    
    psi.beta.trait ~ dnorm(0,0.001)
	mu.psi.beta ~ dnorm(0,0.001)

    
    for (sp in 1:nsp) {
    	psi.beta.sp[sp] <- mu.psi.beta + psi.beta.trait*trait[sp] + psi.beta.sp.rand[sp]
	}
  ...
  res <- jags.parallel(data=list('X', 'nsp', 'nsite', 'nyr', 'nrep', 
                         'env', 'VCOV', 'ID', 'trait', 'MU.lambda', 'TAU.lambda'),
                       inits=d$inits,
