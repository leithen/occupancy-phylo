run.R2jags.model <- function(d,
  ...
  cat('model {
	...
	for (mod in 1:nmods){
    		tau.p.0[mod] <- 1/(sigma.p.0[mod]*sigma.p.0[mod])
	}
	
    ## species-specific detectability
    for(sp in 1:nsp) {
      p.0[sp] ~ dnorm(mu.p.0[IND+1],tau.p.0[IND+1])
      logit(p[sp]) <- p.0[sp]
    }
	...
	for (mod in 1:nmods){
	    tau.psi.site[mod] <- 1/(sigma.psi.site[mod]*sigma.psi.site[mod])
	}

    for (site in 1:nsite) {
      psi.site[site] ~ dnorm(0, tau.psi.site[IND+1])
    }

    ## random effect of year on occupancy
    ## mu.psi.year ~ dnorm(0,0.001)
	for (mod in 1:nmods){
	    tau.psi.year[mod] <- 1/(sigma.psi.year[mod]*sigma.psi.year[mod])
	}
	
	
    for (yr in 1:nyr) {
      psi.year[yr] ~ dnorm(0, tau.psi.year[IND+1])
    }
    ...
    ## Make sure psi.0.sp is saved as matrix with nsp rows and nmods cols
    
    for(sp in 1:nsp) {
      psi.0[sp] <- psi.0.sp[sp, IND+1] ## + psi.0.trait*trait[sp] ## No trait for now!
    }
    ...
    for(sp in 1:nsp) {
      mu.beta.sp[sp] <- mu.psi.beta[IND+1] ## + beta.trait*trait[sp] ##No traits for now!
    }
	...
	for (mod in 1:nmods){    
    		logit(lambda[mod]) <- logit.lambda[mod]
    }
    ...
    tau.beta.sp.mat[1:nsp,1:nsp] <-inverse((sigma.psi.beta[IND+1])^2*beta.mat[,])
  ...
  res <- jags.parallel(data=list('X', 'nsp', 'nsite', 'nyr', 'nrep', 
                         'env', 'VCOV', 'ID', 'trait','mu.p.0', 
                         'sigma.p.0','sigma.psi.site',	'sigma.psi.year', 
                         'psi.0.sp', 'mu.psi.beta', 'sigma.psi.beta', 
                         'logit.lambda'),
                       inits=d$inits,
