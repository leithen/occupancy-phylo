ms.ms <- function(d,
  ...
  cat('model {
    ...
    p.landcover[1] <- 0
    p.landcover[2] ~ dnorm(0,0.001)
    p.landcover[3] ~ dnorm(0,0.001)
    ...
    for(sp in 1:nsp) {
      ...
      for(site in 1:nsite) {
        ...
        logit(p[site,sp]) <- p.0[sp] + p.landcover[landcover[site]]
        ...
        mu.p[site,1,sp] <- Z[site,1,sp]*p[site,sp]
        ...
        for(yr in 1:(nyear-1)) {
          ...
          mu.p[site,yr+1,sp] <- Z[site,yr+1,sp]*p[site,sp]
