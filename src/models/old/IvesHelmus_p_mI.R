ms.ms <- function(d,
  ...
  cat('model {
    ...
    zsig2.psi.0 ~ dunif(0,100)
    tau.psi.0 <- 1/zsig2.psi.0
    ...
    ## Create tracking variables for the proportion of variance in
    ## species occupancy that comes from phylogeney and other sources 
    
    Prop.Phylo.Spp <- zsig2.psi.phylo / (zsig2.psi.0 +
                                         zsig2.psi.phylo) 
    Prop.Phylo <- zsig2.psi.phylo / (zsig2.psi.0 +
                                     zsig2.psi.phylo +
                                     zsig2.psi.site +
                                     zsig2.psi.year) 
    Prop.Spp <- zsig2.psi.0 / (zsig2.psi.0 +
                               zsig2.psi.phylo +
                               zsig2.psi.site +
                               zsig2.psi.year)
    Prop.Site <- zsig2.psi.site / (zsig2.psi.0 +
                                   zsig2.psi.phylo +
                                   zsig2.psi.site +
                                   zsig2.psi.year)
    Prop.Year <- zsig2.psi.year / (zsig2.psi.0 +
                                   zsig2.psi.phylo +
                                   zsig2.psi.site +
                                   zsig2.psi.year)
