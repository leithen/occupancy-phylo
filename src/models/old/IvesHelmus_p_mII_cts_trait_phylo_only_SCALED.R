ms.ms <- function(d,
  ...
  cat('model {
    ...
    beta.lambdaMat[1:nsp,1:nsp] <- beta.lambda*VCOV[,] + (1-beta.lambda)*ID[,]
    tau.beta.spp.VV[1:nsp,1:nsp] <- inverse(beta.lambdaMat[,])
    ...
    for(sp in 1:nsp) {
      beta.VVphylo.scaled[sp] <-
        ((beta.VVphylo[sp] - mean(beta.VVphylo)) / sd(beta.VVphylo))
    }
    
    ## Standard deviation of random effect of species response to
    ## landcover (incorporating phylogenetic component through lambda)
