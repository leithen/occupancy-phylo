ms.ms <- function(d,
  ...
  cat('model {
    ...
    for(sp in 1:nsp) {
      for(i in 1:2) {
        phi.0[i,sp] ~ dnorm(0,0.001)
        gam.0[i,sp] ~ dnorm(0,0.001)
      }
	}
    ...
    for(i in 1:2) {
     sigma.phi.spp[i] ~ dunif(0.000001, 100)
     sigma.gam.spp[i] ~ dunif(0.000001, 100)
      tau.phi.spp[i] <- 1/(sigma.phi.spp[i]*sigma.phi.spp[i])
      tau.gam.spp[i] <- 1/(sigma.gam.spp[i]*sigma.gam.spp[i])
      ...
      tau.phi.0[i,1:nsp,1:nsp] <-
        tau.phi.spp[i]*inverse(phi.lambdaMat[i,,])
      tau.gam.0[i,1:nsp,1:nsp] <-
        tau.gam.spp[i]*inverse(gam.lambdaMat[i,,])

      phi.VVphylo[i,1:nsp] ~ dmnorm(VV.mu[i,], tau.phi.0[i,,])
      gam.VVphylo[i,1:nsp] ~ dmnorm(VV.mu[i,], tau.gam.0[i,,])
    }
    ...
    for(sp in 1:nsp) {
      ...
      for(site in 1:nsite) {
        ...
        logit(phi[site,sp]) <- phi.0[landcover[site],sp] + 
        								phi.VVphylo[landcover[site],sp] + 
        									phi.site[site]
        
        logit(gam[site,sp]) <- gam.0[landcover[site],sp] + 
        									gam.VVphylo[landcover[site],sp] + 
        										gam.site[site]
  ...
  jags.parallel(data=list('X', 'nsp', 'nsite', 'nyear', 'nrep',
                  'date.mat', 'landcover', 'frac.presence',  'VCOV', 'ID', 'VV.mu'),
                inits=d$inits,
