ms.ms <- function(d,
  ...
  cat('model {
	...
	## Random landcover slope of species
      ...
      mu.gam.landcover ~ dnorm(0,0.001)
      sigma.phi.landcover ~ dunif(0,100)
      sigma.gam.landcover ~ dunif(0,100)
      tau.phi.landcover <- 1/(sigma.phi.landcover*sigma.phi.landcover)
      tau.gam.landcover <- 1/(sigma.gam.landcover*sigma.gam.landcover)
    

    for(sp in 1:nsp) {
        phi.landcover[sp] ~ dnorm(mu.phi.landcover,tau.phi.landcover)
        gam.landcover[sp] ~ dnorm(mu.gam.landcover,tau.gam.landcover)
      }
    ...
    for(sp in 1:nsp) {
      ...
      for(site in 1:nsite) {
        ...
        logit(gam[site,sp]) <- gam.0[sp] + 
        									gam.landcover[sp]*landcover[site] +
