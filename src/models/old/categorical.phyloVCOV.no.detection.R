ms.ms <- function(d,
  ...
  cat('model {
    ...
    for(i in 1:2) {
      ...
      for(sp in 1:nsp) {
        mu.phi.spp.0[i,sp] <- mu.phi.0[i]
        mu.gam.spp.0[i,sp] <- mu.gam.0[i]
      }

      tau.phi.spp[i] ~ dgamma(1,1)
      tau.gam.spp[i] ~ dgamma(1,1)

      phi.lambda[i] ~ dunif(0,1)
      gam.lambda[i] ~ dunif(0,1)
      ...
      phi.0[i,1:nsp] ~ dmnorm(mu.phi.spp.0[i,], tau.phi.0[i,,])
      gam.0[i,1:nsp] ~ dmnorm(mu.gam.spp.0[i,], tau.gam.0[i,,])
