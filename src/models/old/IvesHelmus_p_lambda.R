ms.ms <- function(d,
  ...
  ## species-specific detectability
	...
	## Single landcover slope across species
 ...
 ##  for(sp in 1:nsp) {
 ##      psi.landcover[sp] ~ dnorm(mu.psi.landcover, tau.psi.landcover)
 ##     }
    ...
    for(sp in 1:nsp) {
      ...
      for(site in 1:nsite) {
          ...
          logit(psi[site,yr,sp]) <-psi.0[sp] + 
        									mu.psi.landcover*landcover[site] +
        										psi.VVphylo[sp]*landcover[site] +
        											psi.site[site]
